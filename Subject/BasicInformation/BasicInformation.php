...
<script type="text/javascript">
	...
	function addpopup(){
		window.open("./Subject/BasicInformation/addpopup.php","추가","width=500,height=400,left=500,top=100");
	}
	// add버튼 이벤트
	...
	$('#table_contents').on('click','tr',function(evt){
			...
			if($cell.index()>0&&$cell.index()<9){
			    ...
			    mapForm.action = "./Subject/BasicInformation/modifypopup.php";
	...
	function deleteitem(){
		...
		if(arr.length!=0){
			...
			$.ajax({
					...
					//dataType:"json",
					data : datastring,
					url:"./Subject/BasicInformation/delete.php",
	...
	function search(){
		...
		$.ajax({
				...
				dataType:"json",
				data : search_data,
				url:"./Subject/BasicInformation/search.php",
				success: function(data){
					console.log(data);
					var text="";
	...
	function show_default_table(){
		$.ajax({
				type:"POST",
				dataType:"json",
				url:"./Subject/BasicInformation/default_table.php",
				success: function(data){
					console.log(data);
					var text="";
					for(var i=0;i<data.length;i++){
