...
<script type="text/javascript">
	...
	function addpopup(){
		window.open("./Setting/Professor/addpopup.php","추가","width=500,height=400,left=500,top=100");
	}
	// add버튼 이벤트
	...
	$('#table_contents').on('click','tr',function(evt){
			...
			if($cell.index()>0&&$cell.index()<7){
				console.log($(this).find('input').val());
				
				var mapForm = document.createElement("form");
			    mapForm.target = "Map";
			    mapForm.method = "POST"; // or "post" if appropriate
			    mapForm.action = "./Setting/Professor/modifypopup.php";
	...
	function deleteitem(){
		...
		if(arr.length!=0){
			...
			$.ajax({
					...
					//dataType:"json",
					data : datastring,
					url:"./Setting/Professor/delete.php",
	...
	function search(){
		...
		$.ajax({
				...
				dataType:"json",
				data : search_data,
				url:"./Setting/Professor/search.php",
				success: function(data){
					console.log(data);
					var text="";
	...
	function show_default_table(){
		$.ajax({
				type:"POST",
				dataType:"json",
				url:"./Setting/Professor/default_table.php",
				success: function(data){
					console.log(data);
					var text="";
					for(var i=0;i<data.length;i++){
