...
<script type="text/javascript">
	...
	function addpopup(){
		window.open("./Setting/Major/addpopup.php","추가","width=500,height=400,left=500,top=100");
	}
	// add버튼 이벤트
	...
	$('#table_contents').on('click','tr',function(evt){
			...
			if($cell.index()>0&&$cell.index()<4){
			    ...
			    mapForm.action = "./Setting/Major/modifypopup.php";
	...
	function show_default_table(){
		$.ajax({
				type:"POST",
				dataType:"json",
				url:"./Setting/Major/default_table.php",
				success: function(data){
					console.log(data);
					var text="";
					for(var i=0;i<data.length;i++){
						text+="<tr><th><input type='checkbox' name='select' value='"+data[i].idx+"'></th>"+"<th>"+data[i].MajorName+"</th><th>"+data[i].MajorCode+"</th>"+"<th>"+data[i].count+"</th>"+"<th></th>";
					}
					$("#table_contents").html(text);
				}
			});
