...
<head>
	...
	<?php 
		...
		$sql="select * from campus where idx='".$idx."'";
		...
		if($result->num_rows>0){
			while($row=$result->fetch_assoc()){
				$text_CampusName= $row["CampusName"];
				$text_CampusCode=$row["CampusCode"];
			}
		}else{
			echo "0 result";
		}
...
<script type="text/javascript">
		$("#btn_complete").click(function(){
			var json = { idx: idx_num, CampusName: $("#CampusName").val(), CampusCode : $("#CampusCode").val()};
			console.log(json);
			var url="./modify.php";
			$.ajax({
				url: url,
...
<script type="text/javascript">
		...
		var text_CampusName= '<?= $text_CampusName ?>';
		var text_CampusCode= '<?= $text_CampusCode ?>';
		
		$("#CampusName").val(text_CampusName);
		$("#CampusCode").val(text_CampusCode);
