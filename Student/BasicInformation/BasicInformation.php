...
<script type="text/javascript">
	...
	function addpopup(){
		window.open("./Student/BasicInformation/addpopup.php","추가","width=500,height=400,left=500,top=100");
	}
	// add버튼 이벤트
	...
	$('#table_contents').on('click','tr',function(evt){
			...
			if($cell.index()>0&&$cell.index()<12){
			    ...
			    mapForm.action = "./Student/BasicInformation/modifypopup.php";
	...
	function deleteitem(){
		...
		if(arr.length!=0){
			...
			$.ajax({
					...
					//dataType:"json",
					data : datastring,
					url:"./Student/BasicInformation/delete.php",
	...
	function search(){
		...
		$.ajax({
				...
				dataType:"json",
				data : search_data,
				url:"./Student/BasicInformation/search.php",
				success: function(data){
					console.log(data);
					var text="";
					for(var i=0;i<data.length;i++){
						text+="<tr><th><input type='checkbox' name='select' value='"+data[i].idx+"'></th>"+"<th>"+data[i].StudentNumber+"</th><th>"+data[i].FirstName+"</th><th>"+data[i].LastName+"</th><th>"+data[i].Birth+"</th><th>"+data[i].Phone+"</th><th>"+data[i].Email+"</th><th>"+data[i].Address+"</th><th>"+data[i].MajorName+"</th><th>"+data[i].State+"</th><th>"+data[i].CampusName+"</th><th>"+data[i].Level+"</th>"+"<td></td>";
					}
	...
	function show_default_table(){
		$.ajax({
				type:"POST",
				dataType:"json",
				url:"./Student/BasicInformation/default_table.php",
				success: function(data){
					console.log(data);
					var text="";
					for(var i=0;i<data.length;i++){
